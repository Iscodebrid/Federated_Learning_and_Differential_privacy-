...
def moments_calcu(max_lmbd, q, sigma, epsilon):
    lmbds = range(1, max_lmbd + 1)
    for T in range(1, 10000):
        # 计算每轮的隐私损失
        log_moments = []
        for lmbd in lmbds:
            log_moment = T * compute_log_moment(q, sigma, T, lmbd)
            log_moments.append((lmbd, log_moment))

        # 计算隐私泄露风险
        eps, delta = get_privacy_spent(log_moments, target_eps=epsilon)
        if delta >= 1e-5:
            print(T)
            return T
...
if __name__ == '__main__':
    ...
    moments_calcu(max_lmbd=32, q=0.01, sigma=0.9, epsilon=2.0)
