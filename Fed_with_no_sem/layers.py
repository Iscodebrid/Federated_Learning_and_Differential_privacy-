...
class DiffusionModel(nn.Module):
    def __init__(self, embed_dim, timesteps=1000, *args, **kwargs):
        super().__init__(DiffusionModel, self).__init__()
    ...
    def forward(self, x, t):
        ...
        alpha_t = self.compute_alpha(t)
        noisey_x = torch.sqrt(alpha_t) * x + torch.sqrt(1-alpha_t) * noise
        pred_noise = self.model(noisey_x)
        return pred_noise

    def compute_alpha(self, t):
        # 计算扩散过程中的 alpha_t 参数
        beta = torch.linspace(0.0001, 0.02, self.timesteps)
        alpha = 1 - beta
        alpha_bar = torch.cumprod(alpha, dim=0)
        return alpha_bar[t].view(-1, 1, 1)
